      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <nav className="flex items-center space-x-2 text-sm bg-[#212121]/30 w-fit px-5 py-2.5 rounded-full border border-zinc-800">
          ...
          <div className="flex items-center">
            <span className="text-white font-semibold tracking-wide">Hero Section</span>
          ...
          <Table>
            <TableHeader className="bg-zinc-900/60">
              <TableRow className="border-zinc-800 hover:bg-transparent">
                <TableHead className="text-zinc-400 py-6 pl-8 font-bold uppercase text-xs tracking-widest">Preview</TableHead>
                <TableHead className="text-zinc-400 py-6 font-bold uppercase text-xs tracking-widest">Main Content</TableHead>
                <TableHead className="text-zinc-400 py-6 font-bold uppercase text-xs tracking-widest">Status & Toggle</TableHead>
                <TableHead className="text-zinc-400 py-6 pr-8 text-right font-bold uppercase text-xs tracking-widest">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {heroes.map((hero) => (
                <TableRow key={hero.id} className="border-zinc-800/40 hover:bg-[#c7d300]/5 transition-all duration-500 group">
                  ...
                  <TableCell className="py-8">
                    ...
                    <div className="text-sm text-zinc-500 mt-1 font-medium">{hero.titleLine2}</div>
                    <div className="flex flex-wrap gap-2 mt-3">
                      {hero.typingSequence.map((text, i) => (
                        <span key={i} className="bg-zinc-900 text-zinc-400 text-[11px] px-3 py-1 rounded-md border border-zinc-800 font-mono">
                          {text}
                        </span>
                      ))}
                    </div>
